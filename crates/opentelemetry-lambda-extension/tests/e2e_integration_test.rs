...
async fn test_e2e_trace_propagation_and_platform_telemetry() {
    ...
    // covers the whole test. Both the extension and the function runtime
    // accept the scheme-prefixed form.
    async_with_vars(
        [(
            ...
            collector
                .with_collector(|c| {
                    ...
                    if c.span_count() == 0 {
                        ...
                        println!(
                            "This may indicate the extension didn't receive/forward the spans"
                        );
                        return;
                    }
